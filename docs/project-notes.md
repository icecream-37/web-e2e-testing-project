...
1. 补充更多边界场景，例如 Last Name 为空、Postal Code 为空、移除购物车商品。
2. 引入 Page Object Model，把页面操作封装成页面类，提高脚本复用性。
3. 增加 JSON 或 JUnit 报告，方便后续接入 CI 平台。
4. 接入 GitHub Actions，实现提交代码后自动运行测试。
5. 增加多浏览器测试，覆盖 Chromium、Firefox 和 WebKit。
